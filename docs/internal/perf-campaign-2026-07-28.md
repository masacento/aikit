...
| ~~39~~ | ~~WAND / block-max WAND / MaxScore dynamic pruning~~ — **WAND DONE for `bm25` (3.88×); built, measured and REVERTED for `sparse`** (§7.37); **MaxScore (the long-query >8-term case) built, measured, REVERTED 2026-07-31** — exact (== exhaustive over 1000 queries, mutation-checked) but 2.5–5.7× SLOWER: the exhaustive TAAT accumulator (postings once into scores[]) beats DAAT pruning when selectivity is uniform; MaxScore needs skewed impacts (SPLADE weights) which nothing here measures | bm25, sparse | plus a 1.40–8.47× `sparse` win from porting item 44 | = | — |
| 40 | Flash-attention / online-softmax tiling | encoder, vision | 1.05–1.15× alone; 67 MB → 1 MB scratch — **VISION-scoped (scope-checked 2026-07-31): the text encoder right-truncates to `maxSeq`≤512, so its `[L×L]` scores scratch is already ≤1 MB; the 67 MB is L≈4096, i.e. the Qwen ViT patch path. Not a text/code-retrieval lever.** | ≠ | L |
