#!/usr/bin/env python3
"""pin_encoder.py — produce testdata/encoder_golden.json from the real
...
Usage (from repo root):
    .venv/bin/python scripts/oracle/pin_encoder.py
    # writes testdata/encoder_golden.json directly (no copy step needed).
"""
...
# Mandatory query prefix from the model card; mirrors encoder.QueryPrefix.
...
def main() -> int:
    ...
    out_path = REPO_ROOT / "testdata" / "encoder_golden.json"

    sys.stderr.write("[pin_encoder] loading nomic-ai/CodeRankEmbed (CPU; deterministic) ...\n")
    sys.stderr.flush()
    ...
    with out_path.open("w") as f:
        ...
        f.write("\n")
    sys.stderr.write(f"[pin_encoder] wrote {out_path.relative_to(REPO_ROOT)} ({out_path.stat().st_size/1024:.1f} KB)\n")
    return 0
