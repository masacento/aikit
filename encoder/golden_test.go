...
const goldenPath = "../testdata/encoder_golden.json"
...
func TestGoldenFixture_tokenizerParity(t *testing.T) {
	...
	if _, err := os.Stat(tokenizerPath); errors.Is(err, fs.ErrNotExist) {
		t.Skipf("no tokenizer at %s — symlink testdata/encoder-model -> HF snapshot", tokenizerPath)
	}
...
func TestGoldenFixture_queryPrefixIsLiteral(t *testing.T) {
	...
	if qCase == nil || dCase == nil {
		t.Skip("fixture missing the paired prefix-equivalence cases; regenerate pin_encoder.py")
	}
