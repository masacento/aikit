...
func (e *encoder) proj(src gpu.Buffer, m mat, bias, dst gpu.Buffer, M int) error {
	...
	if m.quant {
		...
		if err := e.q.Launch(e.k.GEMMW8A8, gpu.Grid1D(M*N, 256),
	...
	} else if err := e.q.Launch(e.k.GEMMF32, gpu.Grid1D(M*N, 256),
...
func (e *encoder) ForwardViT(pixelValues []float32, gridTHW [][3]int) ([]float32, error) {
	...
	if err := e.q.Launch(e.k.GEMMF32, gpu.Grid1D(n*H, 256),
