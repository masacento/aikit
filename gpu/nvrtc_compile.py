...
compiler, and a modern host libstdc++ breaks nvcc's cicc; NVRTC needs no host compiler
at all, and the RUNTIME needs no CUDA toolkit — only libcuda, which JITs the PTX).
...
Lifted from goinfer/cuda/nvrtc_compile.py — the same build step, so the same script.
"""
